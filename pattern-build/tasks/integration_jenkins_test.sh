...
echo "Beginning of integration_jenkins script"
...
kitchen_setup(){
    ...
    eval "$(/opt/chefdk/bin/chef shell-init bash)"
    /opt/chefdk/embedded/bin/gem install kitchen-softlayer
...
get_build_env(){
  if [ "${DOCKER_REGISTRY:-}" ] && [ "${DOCKER_REGISTRY_PASS:-}" ]; then
    ...
    echo "curl -O -H "X-JFrog-Art-Api:${DOCKER_REGISTRY_PASS}" https://${DOCKER_REGISTRY}/artifactory/orpheus-local-generic/opencontent/labs/build_env_vars.gpg"
...
if [ "$TRAVIS_PULL_REQUEST" != "false" ] && [ "$TRAVIS_BRANCH" == "development" ]; then
  ...
  echo "Run the jenkins integration tests"
  get_build_env
  #get_travis_keys
  #kitchen_setup
  ...
  rake jenkins_integration:passwords
  diagnose
  ...
  rake jenkins_integration:parallel_run
  rc=$?;
...
echo "End of integration_jenkins script"