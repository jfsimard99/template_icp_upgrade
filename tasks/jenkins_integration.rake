...
namespace :jenkins_integration do
  def randompass
  ...
  task :parallel_run do
    ...
    if File.file?("./.kitchen.sl.yml")
      ...
      cfg = YAML.load_file "./.kitchen.jenkins.sl.yml"
    ...
    cloud_instances = []
    clouds.each do |c|
      if File.file?("./.kitchen.jenkins.#{c}.yml")
        cloud_instances.concat(Kitchen::Config.new(loader: Kitchen::Loader::YAML
                          .new(project_config: "./.kitchen.jenkins.#{c}.yml")).instances)
      end
    end
  ...
  # Utility intended for developers to run locally/in build engine to 
